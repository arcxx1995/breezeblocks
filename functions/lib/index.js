...
exports.submitMove = exports.createMatchIfReady = exports.cancelQueue = exports.joinQueue = void 0;
...
exports.cancelQueue = (0, https_1.onCall)(async (request) => {
...
exports.createMatchIfReady = (0, https_1.onCall)(async (request) => {
    const input = request.data;
    ...
    if (waitingSnapshot.size < input.requestedPlayerCount) {
        return { status: "queued", queueName };
    }
    const selectedQueueDocs = waitingSnapshot.docs;
    const gameRef = db.collection("games").doc();
    const startedAt = firestore_1.Timestamp.now();
    const turnDeadlineAt = firestore_1.Timestamp.fromMillis(startedAt.toMillis() + gameRules_1.TURN_DURATION_SECONDS * 1000);
    const selectedPlayers = selectedQueueDocs.map((doc, index) => queueDocToPlayer(doc.id, doc.data(), index));
    ...
    for (const line of (0, gameRules_1.createInitialLines)()) {
        batch.set(gameRef.collection("lines").doc(line.lineId), line);
    }
    for (const box of (0, gameRules_1.createInitialBoxes)()) {
    ...
    await batch.commit();
    return { status: "matched", gameId: gameRef.id, queueName };
});
exports.submitMove = (0, https_1.onCall)(async (request) => {
    const input = request.data;
    const uid = requireUid(request.auth?.uid);
    if (!input.gameId)
    ...
    const result = await db.runTransaction(async (transaction) => {
        ...
        const players = playerSnapshots.docs.map((doc) => doc.data());
        ...
        const lines = new Map(lineSnapshots.docs.map((doc) => [doc.id, doc.data()]));
        const boxes = new Map(boxSnapshots.docs.map((doc) => [doc.id, doc.data()]));
        lines.set(targetLineId, { ...line, ownerPlayerId: currentPlayer.playerId });
        const completedBoxIds = (0, gameRules_1.getCompletedBoxes)(input.orientation, input.row, input.col, lines, boxes);
        ...
        transaction.update(gameRef.collection("players").doc(currentPlayer.playerId), {
            score: firestore_1.FieldValue.increment(completedBoxIds.length),
            consecutiveSkips: 0,
        ...
        const capturedBoxCount = boxSnapshots.docs.filter((doc) => doc.data().ownerPlayerId).length +
            completedBoxIds.length;
        const gameComplete = capturedBoxCount >= gameRules_1.BOX_ROWS * gameRules_1.BOX_COLS;
...
function calculateWinnerIds(players, scoringPlayerId, gainedBoxes) {
